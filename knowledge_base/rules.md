...
### Skill selection and recommendations
...
- If no installed skill adequately covers a domain where specialized guidance would genuinely improve correctness, security, reliability, performance, UX, DX, observability, or maintainability, recommend a specific additional skill to the developer and explain the concrete quality gap it would address.
...
- Present the best-fit skill, rationale, source link, and exact install command for developer review. Never install a recommended skill without the developer's explicit approval.
