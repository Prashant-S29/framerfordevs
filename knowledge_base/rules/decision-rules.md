...
Decision records are domain references, not mandatory chronological reading. Use `knowledge_base/decisions/index.md` to select a prior decision only when current work changes, consumes, or must preserve its contracts, invariants, authority, security model, or trade-offs. Start from current code/tests and follow real dependencies to additional records; do not preload unrelated milestone decisions. Add or update the index whenever a decision is created, superseded, or changes its read conditions.
