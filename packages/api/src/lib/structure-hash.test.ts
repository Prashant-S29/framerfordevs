...
import { CollectionFieldId, CollectionId, SchemaRevisionId } from "../contracts/schemas";
...
function typedCollection(value: unknown) {
  ...
  if (!result.valid)
    throw new Error(`Invalid structure-hash fixture: ${JSON.stringify(result.issues)}`);
