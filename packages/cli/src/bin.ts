...
import { CliConfigFileSystem, CliConfigFileSystemLive, loadCliConfig } from "./config";
  ...
  if (command === "schema check") {
    return Effect.gen(function* () {
      ...
      const locked = yield* readBoundedJson(
        join(loaded.directory, ".framerfordevs", "schema.lock.json"),
        SchemaLock,
        16 * 1_024 * 1_024,
      );
  ...
  if (command === "" || command === "help") {
    return Effect.succeed({
      ...
      usage: [
        ...
        "ffd link --api <origin> --project <id> --environment <key> [--output <path>]",
        "ffd schema pull",
...
async function main() {
  ...
  if (result._tag === "Success") {
    ...
    const response = { ok: false, code: taggedCode ?? messageCode ?? "CLI_ERROR" };
    process.stderr.write(
      json
        ? `${JSON.stringify(response)}\n`
