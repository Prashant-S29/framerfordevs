...
import { prepareEditorCommand, type EditorPreparationDependencies } from "./index";
...
describe("editor pre-auth command", () => {
  ...
  it("keeps credential and network modules outside the static pre-auth graph", async () => {
    ...
    const pending = [join(sourceRoot, "editor/bin/index.ts")];
    ...
    while (pending.length > 0) {
      ...
      for (const specifier of imports) {
        ...
        const moduleCandidate = `${unresolved}.ts`;
        pending.push(
          await access(moduleCandidate).then(
            () => moduleCandidate,
    ...
    expect(relative).not.toContain("credential-store/index.ts");
    expect(relative).not.toContain("oauth-device/index.ts");
    expect(relative).not.toContain("tooling-http-client/index.ts");
    expect(relative).not.toContain("editor/online/index.ts");
    expect(relative).not.toContain("content/online/index.ts");
    expect(relative).not.toContain("schema/authoring/online/index.ts");
  });
});
